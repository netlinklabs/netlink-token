...
### Debugging (temporary — revert once resolved)
- `index.html`: the "Integrates with" section (`#technology`) was temporarily changed from the animated, duplicated `animate-marquee` row to a plain static `flex flex-wrap` row of the 4 unique logos, with no `transform` animation and no gradient fade overlays. This is a live diagnostic to isolate whether the `transform`-driven marquee animation itself is what keeps the logos from painting on the affected device (see PRs #21, #22 for prior attempts: raw.githubusercontent.com URL and `assets/carousel` path naming were both ruled out). **If this static version renders correctly, restore the marquee animation with a properly isolated fix instead of shipping this as final** — it's meant to gather a data point, not to be the permanent design.
